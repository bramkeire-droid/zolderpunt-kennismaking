...
## IDEE-10 — Meerdere notities tegelijk + opgeslagen notities kunnen wijzigen
- **Datum**: 2026-08-27
- **Bram zei letterlijk**: "Ik heb de notitieknop tijdens het gesprek getest. Ik wil verschillende notites willen kunnen typen zonder de vorige al te moeten opslaan. Ik moet opgeslaan (op enter gedrukt) ook nog kunnen wijzigen."
- **Waar ik mee bezig was**: net klaar met de kostenanalyse (KOSTEN.md)
- **Status**: afgewerkt (2026-08-27) — (a) knop "Nog een notitie" opent een extra kladje; elk kladje heeft eigen soort-knoppen en verdwijnt pas bij bewaren; kop toont "N niet bewaard"; onopgeslagen kladjes overleven een refresh via localStorage per gesprek. (b) Nieuwe PostItRij-component: tekst wijzigen, soort omzetten, verwijderen met bevestiging in de rij (geen browser-dialoog tijdens een gesprek). Werkt tijdens én na het gesprek. Datalaag: wijzigNotitie() + verwijderNotitie(); geen migratie nodig (RLS was al FOR ALL).
- **Live geverifieerd** op de publieke app (bundle index-CCVet1xP.js): twee kladjes tegelijk met tekst en teller "2 niet bewaard"; na opslaan van het eerste bleef het tweede staan; tekst gewijzigd + soort omgezet naar beslissing → beslissingenregister ging 16 → 17; verwijderd → terug 16. Brams eigen 5 notities op ZL-0116 ongemoeid.
