...
| 2026-08-27 | Historiek-filters | PR #4 gemerged en gepubliceerd (bundle index-DpTE0A6G.js). Vier filters op de Historiek-knop: adressen (meerdere), datumbereik, mailbox, richting — werkend in zowel de gratis telling als de verwerking. Loket v9. | **Live geverifieerd op de publieke app**: filterpaneel opent, telling geeft "33 oude mails binnen je afbakening — geschatte AI-kost ± €0,49" met verdeling 2025:3 / 2026:30 en datumbereik. Loket-tests: 106 kandidaten, 2025+2026 = 80+26, in+uit = 63+43, belhouse+zolderpunt = 105+1 — alle drie exact 106. 88/88 tests groen. Nul AI-calls verbruikt. | **Twee stille bugs gevonden en gefixt.** (1) `.in()` met 100 graph_message_ids (152 tekens elk) = URL van ~15.000 tekens → faalde op de gateway, fout werd genegeerd → dezelfde zoekopdracht gaf 33 mails i.p.v. 106 (69% verlies, geen signaal). Nu batches van 20 + foutrapportage; preventief ook `verrijk()` gebatcht (email_cc/call_deelnemers waren ongebatcht tot 500 uuid's). (2) Graph `$search` leest `"a OR b"` als letterlijke tekst → bij twee adressen bijna geen resultaat. Nu één zoekopdracht per (mailbox, adres). **Les: elke `.in()` met lange sleutels moet klein gebatcht én foutgecontroleerd.** |
| 2026-08-27 | Notities bruikbaar maken | Na Brams eerste echte testgesprek (ZL-0116, 4 min, 5 post-its): meerdere kladjes tegelijk + bewerkbare/verwijderbare post-its. PR #5 gemerged en gepubliceerd (bundle index-CCVet1xP.js). Ook: kostenindicatie op de historiek-knop gecorrigeerd (stond 3x te hoog) en KOSTEN.md toegevoegd met het volledige overzicht per functie. | Live doorlopen op de publieke app: 2 kladjes tegelijk → teller "2 niet bewaard" → eerste opgeslagen, tweede bleef staan → tekst gewijzigd en soort omgezet → beslissingenregister 16→17 → verwijderd → terug 16. 88/88 tests groen. Brams eigen notities intact. | Kostenindicatie was 0,015/mail (uit de tijd van 8000-tekens bodies); nagerekend ~0,004. Historiek-classificatie kapte de body nog op 6000 tekens zonder citaat-strip terwijl mail-sync dat al wel deed — nu gelijkgetrokken op 3000. **Les: als je één codepad optimaliseert, controleer of de zusterfunctie dezelfde behandeling kreeg.** |
