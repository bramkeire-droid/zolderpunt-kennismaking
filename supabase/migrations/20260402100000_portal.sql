-- =============================================
-- Klantenportaal: portal velden + events tabel
-- =============================================

-- Portal status & token op leads
ALTER TABLE leads
...
-- Index op portal_token voor snelle lookups
CREATE INDEX IF NOT EXISTS idx_leads_portal_token ON leads (portal_token);

-- Portal events tabel (behavioral tracking)
...
-- Index voor snelle queries per lead
...
-- RLS op portal_events: authenticated users (Bram) kunnen alles lezen
ALTER TABLE portal_events ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Authenticated users can read portal events"
  ON portal_events FOR SELECT
  TO authenticated
  USING (true);

-- Anon users mogen events inserten (klant via portaal)
CREATE POLICY "Anyone can insert portal events"
  ON portal_events FOR INSERT
  TO anon, authenticated
  WITH CHECK (true);

-- Portal sessies tabel (email verificatie tokens)
...
-- Anon users mogen sessions lezen/inserten (voor email gate)
CREATE POLICY "Anyone can manage portal sessions"
  ON portal_sessions FOR ALL
  TO anon, authenticated
  USING (true)
  WITH CHECK (true);

-- Rate limiting tabel voor email verificatie pogingen
...
CREATE POLICY "Anyone can manage email attempts"
  ON portal_email_attempts FOR ALL
  TO anon, authenticated
  USING (true)
  WITH CHECK (true);
